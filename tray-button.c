...
#define GPII_COMMAND_ICON    L"ICON"
#define GPII_COMMAND_ICON_HC L"ICON_HC"
#define GPII_COMMAND_TOOLTIP L"TOOLTIP"
#define GPII_COMMAND_DESTROY L"DESTROY"
#define GPII_COMMAND_STATE   L"STATE"
#define GPII_COMMAND_HWND    L"HWND"

// Notifications sent to GPII
#define GPII_MSG_UPDATE     L"UPDATE"
#define GPII_MSG_CLICK      L"CLICK"
#define GPII_MSG_SHOWMENU   L"SHOWMENU"
#define GPII_MSG_MOUSEENTER L"MOUSEENTER"
#define GPII_MSG_MOUSELEAVE L"MOUSELEAVE"
#define GPII_MSG_POSITION   L"POSITION"
...
#define log(FMT, ...) {fwprintf(stderr, L ## FMT L"\n", __VA_ARGS__); fflush(stderr);}
#define fail(FMT, ...) {fwprintf(stderr, L"fail: " L ## FMT, __VA_ARGS__); fwprintf(stderr, L"(win32:%u)\n", GetLastError()); fflush(stderr);}
#ifdef _DEBUG
 ...
 */
BOOL sendToGpii(WCHAR *msg, WPARAM wParam, LPARAM lParam)
{
	debug("sendToGpii(%s,%u,%u)", msg, wParam, lParam);
 ...
 * @param data The data
 */
void gotGpiiMessage(WCHAR *command, WCHAR* data)
{
	debug("gotGpiiMessage(%s,%s)", command, data);

	if (CompareStringOrdinal(command, -1, GPII_COMMAND_ICON, -1, true) == CSTR_EQUAL) {
		setImage(data);
	} else if (CompareStringOrdinal(command, -1, GPII_COMMAND_ICON_HC, -1, true) == CSTR_EQUAL) {
	    if (iconFileHC) {
	...
	} else if (CompareStringOrdinal(command, -1, GPII_COMMAND_TOOLTIP, -1, true) == CSTR_EQUAL) {
	    setToolTip(data);
	} else if (CompareStringOrdinal(command, -1, GPII_COMMAND_HWND, -1, true) == CSTR_EQUAL) {
	    morphicWindow = (HWND)StrToInt(data);
	} else if (CompareStringOrdinal(command, -1, GPII_COMMAND_STATE, -1, true) == CSTR_EQUAL) {
	...
	} else if (CompareStringOrdinal(command, -1, GPII_COMMAND_DESTROY, -1, true) == CSTR_EQUAL) {
...
WCHAR inputBuffer[0xfff] = {0};
int inputLength = 0;

/**
 * An input line from stdin
 */
void gotInput(WCHAR *input, int len) {
	...
	StringCbCat(inputBuffer, sizeof(inputBuffer), input);

	WCHAR *start = inputBuffer;
	WCHAR *eol;

	// Break the buffer into lines.
	while ((eol = StrChr(start, '\n'))) {
		// Terminate at the end of line.
		if (eol > start && *(eol - 1) == L'\r') {
			*(eol - 1) = L'\0';
		} else {
			*eol = L'\0';
		}
		...
		WCHAR *command = start;
		WCHAR *data = StrChr(command, ' ');	
		while (data && iswspace(*data)) {
			*data = L'\0';
			data++;
		}
		
		gotGpiiMessage(command, data);
		start = eol + 1;
	}

	// Move the rest of the buffer to the beginning.
	if (wcslen(start)) {
		WCHAR *t = StrDup(start);
		StringCbCopy(inputBuffer, sizeof(inputBuffer), t);
		LocalFree(t);
	} else {
		*inputBuffer = L'\0';
	}
...
LRESULT CALLBACK buttonWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_CREATE:
		...
		sendToGpii(GPII_MSG_UPDATE, 0, 0);
		break;
	...
	case WM_MOUSEMOVE:
		...
		if (!hasState(STATE_HOVER)) {
			// Inform gpii
			sendToGpii(GPII_MSG_MOUSEENTER, 0, 0);
	...
	case WM_MOUSELEAVE:
		sendToGpii(GPII_MSG_MOUSELEAVE, 0, 0);
	...
	case WM_LBUTTONUP:
		...
		SetForegroundWindow(morphicWindow);

		sendToGpii(GPII_MSG_CLICK, 0, 0);
	...
	case WM_RBUTTONUP:
		...
		sendToGpii(GPII_MSG_SHOWMENU, 0, 0);
		return 0;
...
/**
 * A thread that signals the events[0] when there is input from stdin, then waits for events[0] to be signalled.
 *
 * It looks like async file reading can't be performed in stdin when it's not a console, so this uses
 * the "here's some work, let me know when you're done" pattern.
 */
DWORD WINAPI InputThread(HANDLE events[]) {
	HANDLE input = GetStdHandle(STD_INPUT_HANDLE);

	HANDLE inputEvent = events[0];
	HANDLE inputHandled = events[1];
	int buf;
	DWORD inputLen;
	// Block until there's something in the file
	while (ReadFile(input, &buf, 0, &inputLen, NULL)) {
		// Tell the main thread something is there, and wait for it to process it.
		SignalObjectAndWait(inputEvent, inputHandled, INFINITE, 0);
	};
...
#define A2W(x) _A2W(x)
	...
	// Monitor the availablity of data in stdin.
	HANDLE input = GetStdHandle(STD_INPUT_HANDLE);
	HANDLE inputEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
	HANDLE inputHandled = CreateEvent(NULL, FALSE, FALSE, NULL);
	HANDLE events[2] = {inputEvent, inputHandled};
	CreateThread(NULL, 0, InputThread, events, 0, NULL);
	...
	while (!die) {
		WCHAR inputBuffer[0xfff];
		DWORD inputLen;
		// Wait for stdin or a window message.
		DWORD waitResult = MsgWaitForMultipleObjectsEx(1, &inputEvent, INFINITE, QS_ALLEVENTS, 0);
		switch (waitResult) {
		case WAIT_OBJECT_0:
			// Got input
			memset(inputBuffer, 0, sizeof(inputBuffer));
			if (ReadFile(input, &inputBuffer, sizeof(inputBuffer) - 2, &inputLen, NULL)) {
				gotInput(inputBuffer, inputLen / sizeof(WCHAR));
			} else {
				die = true;
			}
			ResetEvent(inputEvent);
			SetEvent(inputHandled);
			break;

		case WAIT_OBJECT_0 + 1:
			// Got a new window message
	...
	SendMessage(taskbar, WM_WINDOWPOSCHANGED, 0, 0);
